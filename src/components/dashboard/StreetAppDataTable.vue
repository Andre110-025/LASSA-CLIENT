...
async function getData(page = 1) {
  ...
  try {
    const response = await axios.get(
      `getuserstreetlampsads?status=${props.status}&page=${page}`
    );
      ...
      <thead class="border-b-[.0938rem] border-gray-900 bg-slate-50">
        <tr>
          <th
            ...
            :class="{
              'hidden md:table-cell': header === 'EOI Status',
              'hidden sm:table-cell': header === 'App Fee' || header === 'Status',
            }"
          >
            {{ header }}
          </th>
        ...
        <tr v-for="app in applicationInfo.data" :key="app.id" class="even:bg-slate-50">
          ...
          <td class="p-2.5 text-xs xs:text-sm hidden sm:table-cell">
            ...
            <button
              ...
              @click="completePayment(app.payment_id, app.eoi_status)"
              :disabled="app.eoi_status === 'pending'"
              v-else
            >
              Pay Now
            </button>
          ...
          <td class="p-2.5 text-xs xs:text-sm">
            <a target="_blank" :href="`request/street-app-detail/${app.id}`" class="flex gap-2.5 w-fit">
