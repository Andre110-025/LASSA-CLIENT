...
async function getData(page = 1) {
  ...
  try {
    const response = await axios.get(
      `getallmyfirstpartytemporaryads?status=${props.status}&page=${page}`
    );
    ...
    <table class="w-full mt-6">
        ...
        <tr>
          <th
            ...
            :class="{
              ...
              'hidden md:table-cell': header === 'Status' || header === 'Location',
            }"
          >
            {{ header }}
          </th>
      ...
      <tbody>
        <tr v-for="app in applicationInfo.data" :key="app.id" class="even:bg-slate-50">
          ...
          <td class="p-2.5 text-xs xs:text-sm capitalize hidden md:table-cell" v-text="app.site_location"></td>
          ...
          <td class="p-2.5 text-xs xs:text-sm">
            <a target="_blank" :href="`request/temporary-app-detail/${app.id}`" class="flex gap-2.5 w-fit">
