...
console.log("Received Payment UID:", props.paymentUID);
...
const getArrears = async () => {
  try {
    ...
    const response = await axios.get(
      `getsmallformatdetailsarrears/${props.appID}`
    );

    console.log('arrdata:', response);
...
//   const amount = arrearData.value.Total + arrearData.value.ChargeFee;
...
//       permitFee: arrearData.value.Total,
//       appType: "SmallFormat Details",
//     },
...
const startPartCredoPayment = () => {
  if (partAmount.value > arrearData.value.Total) {
...
function walletPayment() {
  const { open, close } = useModal({
    component: SmallFormatWallet,
    attrs: {
      onConfirm() {
        close();
        getArrears();
      },
      paymentUID: props.paymentUID,
...
const handlePartWalletPayment = async () => {
  ...
  try {
    const balance = Number(userStore.walletDetails.small_format_wallet);
...
const submitPartWalletPayInfo = async () => {
  ...
  const infoWallet = {
    reference_id: `wa${props.paymentUID}`,
    payment_id: props.paymentUID,
    form_type: "Small Format Wallet",
    payment_type: "Wallet",
  ...
  <div v-else>
    <div class="flex py-6 gap-2" style="flex-direction: column">
      <div>
        ...
        >
          Small Format Permit Cost/Arrears
        </h6>
      </div>
    ...
    <div
      v-if="!showPart && arrearData?.Total"
    ...
    >
      ...
      <button
        class="p-3 bg-mainColor text-sm rounded-md text-white grow"
        @click="showPart = true"
        v-if="arrearData?.first_pay && arrearData?.counter < 2"
      >
        Pay Part
      </button>
    </div>
    ...
    >
      ...
      <button
        ...
        class="p-2 bg-mainColor text-sm rounded-md text-white w-full md:w-44"
        @click="handlePayment()"
        :disabled="partAmount < 10000 || partAmount > arrearData.Total"
      >
        Pay with Paystack
      ...
      <template v-if="gateway === 'both'">
        ...
        <button
          class="p-3 bg-mainColor text-sm rounded-md text-white w-[30px] md:w-44"
          @click="payPart()"
          :disabled="partAmount < 10000 || partAmount > arrearData.Total"
        >
          Pay with Paystack
